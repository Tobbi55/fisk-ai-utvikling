...
from app.style import apply_app_style
...
from app.widgets.review_window import ReviewWindow
...
class MainWindow(QMainWindow):  # pylint: disable=too-few-public-methods
    ...
    def __init__(self) -> None:
        ...
        self.parent_layout.setContentsMargins(16, 16, 16, 16)
        self.parent_layout.setSpacing(12)
        ...
        self.review_btn = QPushButton("Review Processed Video")
        self.review_btn.clicked.connect(self.review_processed_video)
        ...
        actions_layout.addStretch()
        self.parent_layout.addLayout(actions_layout)
    ...
    def review_processed_video(self) -> None:
        """Open a processed video and allow deleting false detections."""
        start_dir = settings.save_path if settings.save_path else ""
        video_path = QFileDialog.getOpenFileName(
            self,
            caption="Open processed video",
            directory=start_dir,
        ...
        try:
            dlg = ReviewWindow(Path(video_path), parent=self)
            dlg.exec()
        except Exception as err:  # pylint: disable=broad-except
            ErrorDialog(f"Could not open review window: {err}", parent=self).exec()
